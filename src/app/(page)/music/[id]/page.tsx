...
export default function MusicDetailPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .then(([single, all]) => {
        const data = single?.data ?? (single?.slug ? single : null);
        if (!data) throw new Error(single?.message ?? "Track not found.");
        setTrack(data);
        setAllTracks(Array.isArray(all?.data) ? all.data : []);
  ...
  // ── Fetch activity (likes + comments) on track load ──────────────────────────
  useEffect(() => {
    if (!id) return;
    Promise.all([
      fetch(`${ACTIVITY_URL(id)}/likes?userId=${DEMO_USER_ID}`).then((r) =>
        r.json(),
      ),
      fetch(`${ACTIVITY_URL(id)}/comments`).then((r) => r.json()),
    ])
      .then(([likesRes, commentsRes]) => {
        setLikeCount(likesRes?.data?.count ?? 0);
        setUserLiked(likesRes?.data?.userLiked ?? false);
        setComments(Array.isArray(commentsRes?.data) ? commentsRes.data : []);
        setCommentCount(commentsRes?.total ?? 0);
      })
      .catch(() => {});
  }, [id]);
  ...
  const handleLike = useCallback(async () => {
    ...
    try {
      await fetch(`${ACTIVITY_URL(id)}/like`, {
        method: "POST",
  ...
  const submitComment = useCallback(async () => {
    ...
    setSubmitting(true);
    try {
      const res = await fetch(`${ACTIVITY_URL(id)}/comments`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          userId: DEMO_USER_ID,
  ...
  useEffect(() => {
    if (
      ...
      fetch(`${ACTIVITY_URL(id)}/comments`)
        .then((r) => r.json())
        .then((res) => {
            ...
            <div className="flex-1 min-w-0 pt-1">
              ...
              <div className="flex items-center gap-3 mb-4">
                ...
                >
                  ...
                  >
                    <Heart
                      className={`w-3.5 h-3.5 ${userLiked ? "fill-current" : ""}`}
                      strokeWidth={1.8}
                    />
                  </motion.div>
                  <span>
          ...
          >
            ...
            <div className="flex items-center gap-3 mt-5 pt-5 border-t border-gray-100 dark:border-gray-800">
              ...
              <motion.button
                ...
                onClick={handleLike}
                disabled={likeLoading}
                className={`ml-2 transition-colors ${userLiked ? "text-red-500" : "text-gray-300 dark:text-gray-600 hover:text-red-400"}`}
              >
                <motion.div
                  animate={userLiked ? { scale: [1, 1.4, 1] } : { scale: 1 }}
                  transition={{ duration: 0.3 }}
